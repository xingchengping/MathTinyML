...
/* =====================================================================
 * 5. Sequential Multi-Layer Forward
 * Runs ALL layers in order using the MTML_LAYERS[] table.
 * Generated by export_weights.py alongside model_weights.h.
 * ===================================================================== */
struct MTML_LayerInfo {
    const float* A;
    const float* B;
    uint16_t input_dim;
    uint16_t output_dim;
...
inline void sequential_forward(ArenaAllocator* arena,
                               const MTML_LayerInfo* layers, int num_layers,
                               const float* input, float* output) {
    // Scratch buffers: current input, next output
    const int max_dim = 256;  // adjust if your model is wider
    float buf_a[max_dim];
    float buf_b[max_dim];

    // Load first input
    uint16_t cur_dim = layers[0].input_dim;
    float* cur_data = buf_a;
    for (int i = 0; i < cur_dim; i++) cur_data[i] = input[i];

    for (int li = 0; li < num_layers; li++) {
        const MTML_LayerInfo* L = &layers[li];

        Tensor t_A  = { (float*)L->A, L->output_dim, L->rank };
        Tensor t_B  = { (float*)L->B, L->rank,        L->input_dim };
        Tensor t_x  = { cur_data,      L->input_dim,  1 };

        // Determine output buffer (ping-pong between buf_a and buf_b)
        float* out_data = (li % 2 == 0) ? buf_b : buf_a;
        Tensor t_y  = { out_data,      L->output_dim, 1 };

        low_rank_linear_forward(arena, &t_A, &t_B, &t_x, &t_y);
        arena->reset();  // free intermediate h tensor

        // Apply activation (except last layer)
        if (li < num_layers - 1) {
            apply_activation(&t_y);
        }

        // Next iteration input = this output
        cur_dim = L->output_dim;
        cur_data = out_data;
    }

    // Copy final result
    for (int i = 0; i < cur_dim; i++) output[i] = cur_data[i];
}
